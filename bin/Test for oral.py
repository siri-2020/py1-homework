...
while True:
    ...
    while dish_price < 0:
        price_input = input(
            "Enter price for " + dish_name + " (number only): THB "
        )
...
if len(menu_dictionary) == 0:
    ...
    print("")
    print("--- Menu List (" + str(len(menu_dictionary)) + " items) ---")
    ...
    people_list = []
    print("\nEnter people name (one per line):")
    print("**Press Enter on an empty line to continue**")
    ...
    if len(people_list) == 0:
        ...
        # แสดงรายชื่อทั้งหมด
        print("")
        print("--- People List (" + str(len(people_list)) + " total) ---")
        ...
        for person_name in people_list:
            print("")
            print("What did " + person_name + " eat?")
            ...
            while not got_selected_dishes:
                prompt_text = (
                    "Dishes for " + person_name
                    + " (enter numbers separated by spaces, e.g. 1 2 3): "
                )
                choice_input = input(prompt_text)
        ...
        for dish_name in menu_dictionary:
            ...
            if number_of_eaters > 0:
                ...
                print("  Each person must pays: THB " + str(round(shared_price, 2)))
